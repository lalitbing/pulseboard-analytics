...
function getNonEmpty(key: string) {
...
function normalizeRedisUrl(rawUrl: string) {
  if (/^https?:\/\//i.test(rawUrl)) {
    throw new Error(
      "Redis URL is using http(s). Use the TCP URL (redis:// or rediss://), not UPSTASH_REDIS_REST_URL."
    )
  }

  if (!/^[a-z]+:\/\//i.test(rawUrl)) {
    throw new Error(
      "Redis URL must be a full connection string (redis:// or rediss://). Host-only values are not supported."
    )
  }
...
export function getRedisUrl() {
  ...
  if (directUrl) return normalizeRedisUrl(directUrl)

  const fromParts = buildUrlFromParts()
  if (fromParts) return fromParts

  throw new Error(
    "Redis env is missing. Set REDIS_URL (or UPSTASH_REDIS_URL) to a redis:// or rediss:// connection string."
  )
}

export function getRedisHostSafe(redisUrl: string) {
  try {
